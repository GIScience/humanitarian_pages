...
Downloaded Mapframe: If in the next step the "DOonload PDF" button is clicked, the mapframe is considered a "downloaded mapframe"

Uploaded Sketch Map: AT the moment we are tracking uplaods with in two different ways. 1) A downloaded mapframe has been uploaded by clicking "Digitize your Sketch Maps". Here we are not able to consider multiple-uploads. 2) We save the BB of uploaded Sketch Maps in a databsae and can therefore count these as real upload number. Currently we have the information of geodata downlaods only for approach 1). Soon we will be able to connect this information with approach 2) and plan to dismiss approach 1).

Geodata Downloads: The user download Raster and/or Vector results by clicking "Download ZIP" and/or "Download GeoJSON" (currently only avalaible for uploads tracked through Apache)
...
# Mapframes with downloads and uploads 
...
#olot only downlaods versus uploads
...
Total uploaded Sketch Maps (former blobs) based on database entry from 2024-12-05 on.
...
Analysis is based on Apache log. Uploads = click on upload. This analysis does not consider multiple uplaods. It will be soon replaced by geodata download information on sketches database uplaods.
