  ...
  <div class="h-screen w-full overflow-hidden flex flex-col relative bg-white text-slate-900">
    ...
    <div class="flex-1 flex flex-row relative min-h-0 w-full">
      ...
      <div class="relative h-full flex flex-col transition-[width] duration-700 ease-in-out border-r border-slate-200"
        ...
        <!-- Header -->
        <RiskHeader v-model:selectedCountry="selectedCountry" v-model:selectedDisaster="selectedDisaster"
          :disasters="disasters" :view-mode="viewMode" :is-analysis-visible="showAnalysis" @go-home="goHome"
          @open-about="showAboutModal = true" @toggle-analysis="showAnalysis = !showAnalysis" />

        <!-- Map Canvas -->
        <main id="main-content" class="flex-1 relative overflow-hidden bg-slate-50">
          <RiskMap ref="mapRef" :pmtilesUrl="pmtilesUrl" :pcodeField="pcodeField" :matchArray="matchArray"
            :highlightedPcode="highlightedPcode" :availableCountries="countries.map(c => c.code)"
      ...
      <div class="relative h-full flex flex-col bg-white overflow-hidden transition-[width] duration-700 ease-in-out"
        :class="[
