...
const { file: metaFile } = useMarkdownFile('story-maps', () => route.params.id as string);
const pageTitle = computed(() => metaFile.value?.frontmatter.title as string | undefined);
const pageDescription = computed(() => metaFile.value?.frontmatter.subtitle as string | undefined);
...
const setupObservers = () => {
    ...
    sections.value = headings.map((el) => {
        ...
        used.add(unique);
        el.id = unique;
        return { id: unique, text: el.textContent || '', level: el.tagName === 'H2' ? 2 : 3 };
    });
    activeId.value = '';

    const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const revealTargets = Array.from(contentRef.value.children).filter(
        (el) => !(el as HTMLElement).dataset || (el as HTMLElement).dataset.noReveal === undefined
    ) as HTMLElement[];

    revealTargets.forEach((el) => el.classList.add('opacity-0', 'translate-y-7', 'transition-all', 'duration-700', 'ease-out'));

    if (prefersReducedMotion) {
        revealTargets.forEach((el) => el.classList.remove('opacity-0', 'translate-y-7'));
    } else {
    ...
    if (headings.length || heroEl) {
        sectionObserver = new IntersectionObserver(
            (entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        activeId.value = entry.target === heroEl ? '' : (entry.target as HTMLElement).id;
                    }
                });
            },
...
watch(() => route.params.id, (id) => loadContent(id as string), { immediate: true });
    ...
    >
        <div class="relative">
            <div class="fixed left-0 right-0 top-0 z-[70] h-1 bg-slate-100">
                <div class="h-full bg-gradient-to-r from-heigit-red to-heigit-red-dark transition-[width] duration-150 ease-out"
                    :style="{ width: progress + '%' }">
                </div>
            </div>
            <div v-if="notFound" class="mx-auto max-w-2xl px-4 py-24 text-center sm:px-6 lg:px-8">
                <h1 class="text-2xl font-extrabold text-slate-900">Story map not found</h1>
                <p class="mt-2 text-slate-500">We couldn't find a story map for "{{ route.params.id }}".</p>
            </div>

            <div v-else class="w-full">
                <StoryMapHero ref="heroRef" :id="route.params.id as string" />

                <StoryMapNavbar :sections="sections" :active-id="activeId" @select="onSelectSection" />

                <div ref="contentRef" class="story-content max-w-[1400px] mx-auto pb-24 pt-8 sm:px-6">
