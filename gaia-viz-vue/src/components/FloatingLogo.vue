<template>
  <a 
    href="https://heigit.org" 
    target="_blank" 
    class="absolute bottom-12 right-3 z-40 bg-white/70 backdrop-blur-md border border-slate-200 p-0 rounded-2xl shadow-2xl hover:bg-white transition-all group"
    title="Powered by HeiGIT"
  >
    <img 
      src="https://hot.storage.heigit.org/heigit-hdx-public/oqapi_hdx/logos/heigit-logo-text.svg" 
      alt="HeiGIT Logo"
      class="h-20 w-auto opacity-90 group-hover:opacity-100 transition-opacity"
    >
  </a>
</template>